...
class ClientStore:
    ...
    def save_room(self, name: str, room: Room) -> None:
        rooms = self.load_rooms()
        rooms[name] = room

        payload = {
            room_name: {"invite": stored_room.to_invite()}
            for room_name, stored_room in rooms.items()
        }

        _write_secret(self.rooms_file, json.dumps(payload, indent=2))
